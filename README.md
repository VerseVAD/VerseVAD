...
side-by-side values, equal-poem means, and poem-level dispersion. The six
built-in analysis profiles can be customized after application; local users
may save configuration-only custom profiles without retaining poem text or
...
comparison sets, Lexicon Explorer lookups, and contextual research notes.
Nothing autosaves as a draft. Completed saves are immutable revisions and reopen as their original
...
coverage, distributions, and poem profiles through a Whole Corpus/individual
poem scope selector and concise report-family views. Full profile matrices
remain downloadable. **Form Library** exposes the
...
engine and CSV/Word audit artifacts are available to one-text, corpus,
Personal Corpus, and Lexicon Explorer workflows.
